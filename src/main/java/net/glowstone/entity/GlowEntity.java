...
public abstract class GlowEntity implements Entity {
		...
		if (hasMoved()) {
			...
			if (currentBlock.getType() == Material.ENDER_PORTAL) {
				...
				if (server.getAllowEnd()) {
					...
					if (success) {
						EntityPortalExitEvent e = EventFactory.callEvent(new EntityPortalExitEvent(this,
								previousLocation, location.clone(), velocity.clone(), new Vector()));
	...
	public void setRawLocation(Location location) {
		if (location.getWorld() != world) {
			throw new IllegalArgumentException("Cannot setRawLocation to a different world (got " + location.getWorld()
					+ ", expected " + world + ")");
		}
	...
	public List<Message> createUpdateMessage() {
		...
		boolean teleport = dx > Byte.MAX_VALUE || dy > Byte.MAX_VALUE || dz > Byte.MAX_VALUE || dx < Byte.MIN_VALUE
				|| dy < Byte.MIN_VALUE || dz < Byte.MIN_VALUE;
	...
	/**
	 * Calculate all blocks the entity is intersecting if the entity has a
	 * defined bounding box.
	 * 
	 * @return list of blocks the entity is touching
	 */
	public ArrayList<GlowBlock> getTouchedBlocks() {
		ArrayList<GlowBlock> intersects = new ArrayList<GlowBlock>();
		if (boundingBox == null) {
			...
			for (BlockFace face : new BlockFace[] { BlockFace.EAST, BlockFace.WEST, BlockFace.SOUTH, BlockFace.NORTH,
					BlockFace.DOWN, BlockFace.SELF, BlockFace.NORTH_EAST, BlockFace.NORTH_WEST, BlockFace.SOUTH_EAST,
					BlockFace.SOUTH_WEST }) {
				intersects.add(world.getBlockAt(getLocation().getBlock().getRelative(face).getLocation()));
			...
			for (int x = min.getBlockX(); x <= max.getBlockX(); ++x) {
				for (int y = min.getBlockY(); y <= max.getBlockY(); ++y) {
					for (int z = min.getBlockZ(); z <= max.getBlockZ(); ++z) {
						intersects.add(world.getBlockAt(x, y, z));
					}
				}
			}
		}
