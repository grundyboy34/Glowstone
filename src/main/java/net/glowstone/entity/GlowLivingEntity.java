...
public abstract class GlowLivingEntity extends GlowEntity implements LivingEntity {
	...
	public void pulse() {
		...
		for (PotionEffect effect : effects) {
			...
			if (effect.getDuration() > 0) {
				...
				addPotionEffect(
						new PotionEffect(type, effect.getDuration() - 1, effect.getAmplifier(), effect.isAmbient()),
						true);
			} else {
				// remove
	...
	public <T extends Projectile> T launchProjectile(Class<? extends T> projectile) {
		return launchProjectile(projectile, getLocation().getDirection()); // todo:
																			// multiply
																			// by
																			// some
																			// speed
	...
	public boolean addPotionEffect(PotionEffect effect, boolean force) {
		...
		/*
		 * EntityEffectMessage msg = new EntityEffectMessage(getEntityId(),
		 * effect.getType().getId(), effect.getAmplifier(),
		 * effect.getDuration()); for (Player player :
		 * server.getOnlinePlayers()) { ((GlowPlayer)
	...
	public void removePotionEffect(PotionEffectType type) {
		...
		/*
		 * EntityRemoveEffectMessage msg = new
		 * EntityRemoveEffectMessage(getEntityId(), type.getId()); for (Player
		 * player : server.getOnlinePlayers()) { ((GlowPlayer)
		 * player).getSession().send(msg); }
		 */
	}
