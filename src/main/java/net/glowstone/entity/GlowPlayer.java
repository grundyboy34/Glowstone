...
public final class GlowPlayer extends GlowHumanEntity implements Player {
	 ...
	 * @param profile
	 *            The player's profile with name and UUID information.
	 * @param reader
	...
	public GlowPlayer(GlowSession session, PlayerProfile profile, PlayerDataService.PlayerReader reader) {
		...
		session.send(new JoinGameMessage(SELF_ID, gameMode, world.getEnvironment().getId(), world.getDifficulty()
				.getValue(), session.getServer().getMaxPlayers(), type, false));
		setGameModeDefaults();
	...
	public void pulse() {
		...
		for (GlowEntity entity : world.getEntityManager()) {
			if (entity != this && isWithinDistance(entity) && !knownEntities.contains(entity)
	...
	private void streamBlocks() {
		...
		// done in two steps so that all the new chunks are finalized before any
		// of them are sent
		// this prevents sending a chunk then immediately sending block changes
		// in it because
		...
		final int maxSize = 0x1fff00; // slightly under protocol max size of
										// 0x200000
	...
	private void spawnAt(Location location) {
		...
		// spawn into world
		String type = world.getWorldType().getName().toLowerCase();
		session.send(new RespawnMessage(world.getEnvironment().getId(), world.getDifficulty().getValue(), getGameMode()
				.getValue(), type));
	...
	public void respawn() {
		...
		if (event.getRespawnLocation().getWorld().equals(getWorld()) && knownEntities.size() > 0) {
			// we need to manually reset all known entities if the player
	...
	public void playNote(Location loc, byte instrument, byte note) {
		session.send(new BlockActionMessage(loc.getBlockX(), loc.getBlockY(), loc.getBlockZ(), instrument, note,
	...
	public void playSound(Location location, String sound, float volume, float pitch) {
		if (location == null || sound == null)
			return;
		// the loss of precision here is a bit unfortunate but it's what
		// CraftBukkit does
	...
	@Override
	public void showParticle(Location loc, Particle particle, float offsetX, float offsetY, float offsetZ, float speed,
			int amount) {
		showParticle(loc, particle, null, offsetX, offsetY, offsetZ, speed, amount);
	}

	@Override
	public void showParticle(Location loc, Particle particle, MaterialData material, float offsetX, float offsetY,
		...
		session.send(new PlayParticleMessage(id, longDistance, x, y, z, offsetX, offsetY, offsetZ, speed, amount,
				extData));
	}
	...
	public void sendSignChange(Location location, String[] lines) throws IllegalArgumentException {
		...
		afterBlockChanges.add(UpdateSignMessage.fromPlainText(location.getBlockX(), location.getBlockY(),
				location.getBlockZ(), lines));
	}
	...
	public void sendSignChange(Location location, TextMessage[] lines) {
		...
		afterBlockChanges.add(new UpdateSignMessage(location.getBlockX(), location.getBlockY(), location.getBlockZ(),
				lines));
	}
	 ...
	 * 
	 * @param location
	 *            The location of the block entity.
	 * @param type
	 *            The type of block entity being sent.
	 * @param nbt
	...
	public void sendBlockEntityChange(Location location, GlowBlockEntity type, CompoundTag nbt) {
		...
		afterBlockChanges.add(new UpdateBlockEntityMessage(location.getBlockX(), location.getBlockY(), location
				.getBlockZ(), type.getValue(), nbt));
	}
	 ...
	 * 
	 * @param achievement
	 *            the achievement to award.
	 * @param awardParents
	 *            whether parent achievements should be awarded.
	...
	public void openInventory(InventoryView view) {
		...
		if (viewId != 0) {
			...
			Message open = new OpenWindowMessage(viewId, invMonitor.getType(), title,
					((GlowInventory) view.getTopInventory()).getRawSlots());
			session.send(open);
		}
	...
	public void sendWeather() {
		...
		session.send(new StateChangeMessage(stormy ? StateChangeMessage.Reason.START_RAIN
				: StateChangeMessage.Reason.STOP_RAIN, 0));
	}
	...
	public void showPlayer(Player player) {
		...
		session.send(new UserListItemMessage(UserListItemMessage.Action.ADD_PLAYER, ((GlowPlayer) player)
				.getUserListEntry()));
	}
